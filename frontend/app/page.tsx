...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/api/me/stats/?uuid=${uuid}`)
      .then((r) => r.json())
  ...
  const formationEntries = !puzzle ? null : Object.entries(puzzle.formation);
  const missingPosition = !puzzle
    ? null
    : Object.keys(puzzle.formation).find(
  ...
  async function submitGuess(playerName: string, playerId: number) {
    ...
    const response1 = await fetch(`${BASE_URL}/api/puzzle/${puzzle.id}/guess/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_uuid: uuid, player_id: playerId }),
    });
    ...
    setAttempts((prev) => [
      ...prev,
      { name: playerName, correct: data.solved },
    ]);
    setQuery("");
    setSearchResults([]);
      ...
      <div className="mx-auto flex w-full max-w-[480px] flex-col gap-5">
        <header className="flex items-start justify-between">
          ...
          <div className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-2 text-right">
            ...
            <p className="text-sm font-bold text-[#c8f36b]">
              Puzzle #{puzzle.id}
            </p>
          </div>
        </header>
        ...
        <section className="rounded-2xl border border-white/10 bg-[#101d18] px-4 py-3">
          <p className="text-sm font-bold">
            {puzzle.match_description.home_team}{" "}
            <span className="text-[#82958b]">vs</span>{" "}
            {puzzle.match_description.away_team}
          </p>
          <p className="mt-1 text-xs text-[#82958b]">
            {puzzle.match_description.competition} ·{" "}
            {puzzle.match_description.season}
          </p>
          {puzzle.match_description.description && (
            <p className="mt-1 text-xs font-semibold text-[#c8f36b]">
              {puzzle.match_description.description} ·{" "}
              {puzzle.match_description.formation_type}
            </p>
          )}
        </section>
        ...
        >
          ...
          {missingPosition &&
            (() => {
              ...
              const coords = positionCoords[missingPosition] || {
                x: "50%",
                y: "50%",
              };
              return (
        ...
        {!finished ? (
              ...
              {searchResults.length > 0 && (
                <div className="absolute z-10 mt-2 w-full overflow-hidden rounded-2xl border border-white/10 bg-[#16251e] p-1 shadow-xl">
                  {searchResults.map((player) => (
                    <button
                      key={player.id}
                      onClick={() => submitGuess(player.name, player.id)}
            ...
            >
              <div>
                ...
                <p className="mt-1 text-sm font-bold">
                  {5 - attempts.length} guesses left
                </p>
              </div>
              <div className="flex gap-2">
                {[0, 1, 2, 3, 4].map((index) => (
                  <div
                    key={index}
                    className={`size-3 rounded-full border ${index < attempts.length ? "border-[#c8f36b] bg-[#c8f36b]" : "border-white/20 bg-transparent"}`}
                  />
                ))}
              </div>
            </section>
          ...
          <section className="rounded-[28px] border border-[#c8f36b]/25 bg-[#14271c] p-5 text-center shadow-xl shadow-[#c8f36b]/5">
            <p className="text-3xl font-black text-[#c8f36b]">
              {solved ? "✓" : "✗"}
            </p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              ...
              <div className="rounded-2xl bg-black/15 p-3">
                <p className="text-2xl font-black text-[#c8f36b]">
                  {stats.streak}
                </p>
                <p className="text-[10px] font-bold uppercase tracking-widest text-[#82958b]">
                  Day streak
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-[#82958b]">
              {hints.length} / 3
            </span>
          </div>
